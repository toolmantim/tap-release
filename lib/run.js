...
module.exports = async ({ robot, context, configName }) => {
  let { asset, tap, template, branches } = await getConfig(context, configName, {})
  ...
  if (!owner || !repo || !path || !template) {
    robot.error(`No valid config found`)
    return
  }
  ...
  if (context.event === 'push') {
    ...
    if (branches.indexOf(branch) === -1) {
      robot.error(`Ignoring push. ${branch} is not one of: ${branches.join(', ')}`)
      return
    }
  }
