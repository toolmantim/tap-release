...
module.exports = async ({ app, context, configName }) => {
  ...
  if (asset) {
    if (latest) {
      ...
      if (stableAsset) {
        ...
        app.log(`Calculating hash for ${stableUrl}`)
        renderedTemplate = renderedTemplate
    ...
    if (latestPre) {
      ...
      if (develAsset) {
        ...
        app.log(`Calculating hash for ${develUrl}`)
        renderedTemplate = renderedTemplate
  ...
  app.log(`Updating ${owner}/${repo}/${path}`, { renderedTemplate })
  ...
  app.log(`Updated ${owner}/${repo}/${path}`)
}
